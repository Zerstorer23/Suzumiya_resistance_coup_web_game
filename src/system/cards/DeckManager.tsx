...
export class DeckManager {
  ...
  public static findIndexOfCardIn(
    deck: CardDeck,
    player: Player,
    card: CardRole
  ): number {
  ...
  public static getRandomFromPlayer(
    player: Player,
    deck: CardDeck,
    bias: CardRole = CardRole.None
  ): number | null {
    ...
    //Bias not found.
    const dead: boolean[] = [
      DeckManager.isDead(deck[offset]),
      DeckManager.isDead(deck[offset + 1]),
    ];
    if (!dead[0] && !dead[1]) {
  ...
  public static checkGameOver(ctx: RoomContextType): string {
    ...
    playerMap.forEach((player, id) => {
      ...
      if (
        !this.isDead(deck[player.icard]) ||
        !this.isDead(deck[player.icard + 1])
      ) {
        alive = id;
        numAlive++;
      }
